...
import { Search, MapPin, Star, Users, MessageSquare, Filter, Clock } from 'lucide-react'
import api from '@/lib/api'
import { cn, getInitials } from '@/lib/utils'
import Image from 'next/image'

function AvailabilityPreview({ userId }: { userId: number }) {
  const { data } = useQuery({
    queryKey: ['availability', userId],
    queryFn: () => api.get(`/availability/${userId}`).then(r => r.data),
  })
  const slots = data?.slots || {}
  const DAYS = ['mon','tue','wed','thu','fri','sat','sun']
  const DAY_LABELS = ['M','T','W','T','F','S','S']
  const activeDays = DAYS.filter(d => slots[d])
  if (activeDays.length === 0) return null
  return (
    <div className="mb-3 p-2.5 rounded-xl bg-slate-50 dark:bg-[#1e1e2e]">
      <div className="flex items-center gap-1.5 mb-2">
        <Clock size={11} className="text-brand-500" />
        <span className="text-xs text-[var(--text-muted)] font-medium">Available</span>
      </div>
        ...
        {DAYS.map((d, i) => (
          <div key={d}
            className={cn(
              ...
              slots[d] ? "bg-emerald-500 text-white" : "bg-slate-100 dark:bg-[#2a2a3a] text-[var(--text-muted)]"
            )}
            title={slots[d] || 'Not available'}>
            {DAY_LABELS[i]}
          </div>
        ))}
      </div>
      <p className="text-[10px] text-[var(--text-muted)] mt-1.5">
        {activeDays.length} day{activeDays.length !== 1 ? 's' : ''}/week · {data?.timezone || 'UTC'}
      </p>
    </div>
  )
}
...
function MatchCard({ match }: { match: any }) {
  ...
  return (
    ...
    >
      ...
      <AvailabilityPreview userId={match.id} />
      ...
      <div className="flex gap-2">
        ...
        <Link href={`/chat?user=${match.id}`}
          className="btn-primary text-xs py-2 flex-1 text-center flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-[var(--text-muted)]">Discover people to exchange skills with — see their availability before messaging</p>
      </div>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button onClick={() => setShowFilters(!showFilters)}
          className={cn('btn-secondary flex items-center gap-2 text-sm',
            showFilters && 'bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400')}>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <div>
              ...
              <input type="range" min={0} max={80} step={10} value={minScore}
                onChange={e => setMinScore(Number(e.target.value))} className="w-40 accent-brand-500" />
            </div>
          </div>
        </motion.div>
      )}
      ...
      <div className="flex items-center gap-4 mb-6 text-xs text-[var(--text-muted)]">
        <div className="flex items-center gap-1.5"><Users size={13} />{matches.length} matches</div>
        <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-500 inline-block" />Available day</div>
        <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-slate-200 dark:bg-slate-700 inline-block" />Not available</div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <Link href="/profile" className="btn-primary text-sm mt-4 inline-block">Update Profile</Link>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
