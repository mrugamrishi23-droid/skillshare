...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-full" style={{ height: 'calc(100vh - 61px)' }}>
      {/* Sidebar */}
      <div className={cn(
        "w-full sm:w-80 flex-shrink-0 border-r border-[var(--border)] flex flex-col",
        "bg-[var(--bg-card)]",
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center">
                  <span className={cn("text-sm truncate", conv.unread_count > 0 ? "font-semibold text-[var(--text)]" : "font-medium text-[var(--text)]")}>
        ...
        {!activeConv && !activeUser ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-[var(--text-muted)]">Choose from your existing conversations or find a match</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center gap-3 p-4 border-b border-[var(--border)] bg-[var(--bg-card)]">
              <button onClick={() => { setActiveConv(null); setActiveUser(null) }} className="sm:hidden btn-ghost p-1.5">
                ←
              </button>
                    ...
                    >
                      {!isMe && (
                        <div className="w-7 h-7 rounded-full bg-brand-500 flex items-center justify-center text-white font-bold text-xs mr-2 flex-shrink-0 self-end overflow-hidden">
                          {msg.sender_avatar ? <Image src={msg.sender_avatar} alt="" width={28} height={28} className="w-full h-full object-cover" /> : msg.sender_username?.[0]?.toUpperCase()}
                        </div>
                      )}
                      <div className={cn(
